...
function addCoordinates() {
	...
	if (longitude !== "" && latitude !== "") {
		...
		coordinateTextField.value =
			currentCoordinateTextFieldValue +
			"[" +
			utmPrefix +
			longitude +
			"," +
...
function changeCoordinateFormatMode() {
	...
	if (newMode === universalTraverseMercator) {
		console.log("Turning into blocks");
		utmTimezoneSection.style = "display: block";
		utmHemisphereSection.style = "display: block";
	} else {
		console.log("Hiding");
		utmTimezoneSection.style = "display: none";
		utmHemisphereSection.style = "display: none";
	}
...
function convertInsertedCoordinatesToGeoJson() {
	...
	let parsedCoordinates = parseEnteredCoordinates(
		coordinates,
...
function displayGeoJsonOnMap(geoJson) {
	...
	var sqDgArea = calculatePolygonAreaInSquareDegrees(
		jsonMap.features[0].geometry.coordinates[0]
	);
...
function onDownload() {
	...
	download(JSON.stringify(jsonGeoJson), "map" + ".json", "text/plain");
}
